...
func _on_p2p_session_connect_fail(lobby_id, session_error):
	if session_error == 0:
		Globals.display_message(chat_box, 'WARNING: Session failure with ' + str(lobby_id) + ' [no error given].')
	elif session_error == 1:
		Globals.display_message(chat_box, 'WARNING: Session failure with ' + str(lobby_id) + ' [target user not running the same game].')
	elif session_error == 2:
		Globals.display_message(chat_box, 'WARNING: Session failure with ' + str(lobby_id) + ' [local user does not own app / game].')
	elif session_error == 3:
		Globals.display_message(chat_box, 'WARNING: Session failure with ' + str(lobby_id) + ' [target user is not connected to Steam].')
	elif session_error == 4:
		Globals.display_message(chat_box, 'WARNING: Session failure with ' + str(lobby_id) + ' [connection timed out].')
	elif session_error == 5:
		Globals.display_message(chat_box, 'WARNING: Session failure with ' + str(lobby_id) + ' [unused].')
	else:
		Globals.display_message(chat_box, 'WARNING: Session failure with ' + str(lobby_id) + ' [unknown error ' + str(session_error) + '].')
